...
public class HttpParser {
	...
	public byte[] getRequestBody(byte[] source, int length) {
		List<byte[]> httpRequest = getHttpBody(C.HTTP_REQUEST_BEGIN, C.HTTP_BODY_END, source, length);
	...
	public List<byte[]> getResponseBody(byte[] source, int length) {
		List<byte[]> httpResponse = getHttpBody(C.HTTP_RESPONSE_BEGIN, C.HTTP_BODY_END, source, length);
	...
	private String getURIPath(String requestStr) {
		try {
			int startIndex = requestStr.indexOf(C.HTTP_REQUEST_BEGIN) + C.HTTP_REQUEST_BEGIN.length();
